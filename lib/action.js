...
export class Action {
    ...
    _switch_back(window, windows) {
        const all_monitors = this.mode.get(Mode.SWITCH_BACK_WHEN_FOCUSED_ALL_MONITORS)
        const window_monitor = window.get_monitor();
        const window_list = all_monitors
            ? windows.filter(w => w !== window)
            : windows.filter(w => w.get_monitor() === window_monitor && w !== window)
