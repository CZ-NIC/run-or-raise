...
class App {
  ...
  constructor(settings, seat, keymap) {
     ...
     */
    this.accelerators = new DefaultMap(
      shortcut => new Accelerator(shortcut, this),
    )
     ...
     * @type {DefaultMap<String,Accelerator>} {shortcut => [action, ...]}
     */
    this.layered_accelerators = new DefaultMap(
      shortcut => new Accelerator(shortcut, this),
    )
  ...
  enable() {
    ...
    this.accelerators.forEach(accelerator => {
      ...
      if (
        accelerator.some(action =>
          action.get_state().every(lock => lock === null),
        )
      ) {
        // these are always on
        accelerator.connect()
      } else {
        // these are lock-dependent, on only if keyboard-locks match
    ...
    const on_state_changed = () => {
      ...
      if (!arraysEqual(state, last_state)) {
        lock_dependent_accelerators.forEach(acc =>
          acc.on_state_changed(last_state),
        )
        last_state = state
      }
    }

    this.handler_state_changed = this.keymap.connect(
      "state-changed",
      on_state_changed,
    )
    on_state_changed()
  ...
  _fetch_shortcuts() {
    ...
    try {
      ...
      this.display(
        `Cannot load confpath ${conf_path}, creating new file from default`,
      )
      // instead of using `mkdir -p` and `cp`,
      // the GNOME team required me to use this dark and cumbersome methods to copy a single file
      const target_dir = Gio.File.new_for_path(
        conf_path.substr(0, conf_path.lastIndexOf("/")),
      )
  ...
  get_state() {
    ...
    return [
      this.keymap.get_num_lock_state(),
      this.keymap.get_caps_lock_state(),
      this.keymap.get_scroll_lock_state
        ? this.keymap.get_scroll_lock_state()
        : 0,
    ]
  }
  ...
  layered_mode_start(candidates) {
    candidates.map(action => {
      ...
      if (this.accelerators.has(new_action.shortcut)) {
        this.unblock_later.add(
          this.accelerators.get(new_action.shortcut).block(),
        )
      }
      this.layered_accelerators
        .get(new_action.shortcut)
        .push_layered(new_action)
    })
   ...
   */
  layer_finished() {
    this.layered_accelerators.forEach(temp_accelerator =>
      temp_accelerator.disconnect(),
    )
